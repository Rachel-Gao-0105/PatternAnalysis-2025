...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler, epoch=None, total_epochs=None):
    ...
    for x, y in loader:
        ...
        # Dynamically adjust MixUp alpha (stronger early, weaker later)
...
def evaluate(model, loader, criterion, device):
    """Evaluate model performance and tune threshold based on ROC curve."""
    model.eval()
    losses, preds, gts = [], [], []
    for x, y in loader:
        x = x.to(device, non_blocking=True)
    ...
    acc = accuracy_score(gts, [p >= 0.5 for p in preds])  # default threshold 0.5
...
def parse_args():
    ...
    ap.add_argument("--samples_per_class_train", type=int, default=None, help="Subset of training data per class")
...
def main():
    """Main training pipeline: dataset, model, training, evaluation, plotting."""
    args = parse_args()
