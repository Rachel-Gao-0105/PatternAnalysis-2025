...
def mixup_data(x, y, alpha=0.2):
    # MixUp augmentation: sample lambda ~ Beta(alpha, alpha) and linearly mix pairs
    if alpha <= 0:
        lam = 1.0
        return x, y, y, lam   
    lam = np.random.beta(alpha, alpha)
    bs = x.size(0)
    index = torch.randperm(bs, device=x.device)   # random shuffle indices
    mixed_x = lam * x + (1 - lam) * x[index]      # mix inputs
    y_a, y_b = y, y[index]                        # paired labels
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler, epoch=None, total_epochs=None):
    ...
    for x, y in loader:
        ...
        if epoch is not None and total_epochs is not None:
            ...
            current_alpha = 0.2  # fallback default
        ...
        # Scaled backprop + optimizer step with GradScaler
        scaler.scale(loss).backward()
        scaler.step(optimizer)
        scaler.update()

        losses.append(loss.item())
        p = out.softmax(1)[:, 1].detach().cpu().tolist()  # probability of positive class
        preds.extend(p)
        gts.extend(y.cpu().tolist())

    # Compute training accuracy (using 0.5 threshold) and AUC (approx under MixUp)
...
def evaluate(model, loader, criterion, device):
    # Validation loop with AMP; returns avg loss/acc/auc + tuned acc/threshold
    model.eval()
    losses, preds, gts = [], [], []
    for x, y in loader:
        x = x.to(device, non_blocking=True)
    ...
    acc = accuracy_score(gts, [p >= 0.5 for p in preds]) 
    # Compute AUC; tune threshold via Youden's J (argmax(tpr - fpr))
    try:
        auc = roc_auc_score(gts, preds)
        fpr, tpr, thr = roc_curve(gts, preds)
        j = tpr - fpr
        best_idx = int(np.argmax(j))
        best_thr = float(thr[best_idx])                     # chosen operating point
...
def main():
    ...
    model = ConvNeXt(num_classes=args.num_classes, drop_path_rate=args.drop_path_rate, dropout_rate = args.dropout_rate) #修改
    ...
    # AMP GradScaler (enabled only on CUDA)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Save checkpoint by best val acc@bestThr (tuned)
        ...
        # Save checkpoint by best val acc@0.5
        if val_acc > best_acc:
            best_acc = val_acc
    ...
    print(f"Best val acc: {best_acc:.3f} | best_acc_ckpt: {best_acc_path}")
    print(f"Best val acc@bestThr: {best_acc_tuned:.3f}  | ckpt: {best_acc_tuned_path}")
